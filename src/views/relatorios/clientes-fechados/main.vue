  ...
  <div>
    ...
    <q-card class="q-ma-md animated fadeIn fast">
      ...
      <q-card-section v-else-if="relatorio.length > 0">
        ...
        Clientes Fechados - ${inicio} até ${fim}
        </span>`" />
        ...
        <div class="table-responsive">
          <table class="table">
            <thead>
              <tr>
                <th class="text-center td-relatorio" colspan="2">
                  {{ nomeUnidade }}
                </th>
              </tr>
              <tr>
            ...
            <tbody>
              <tr v-for="(item, index) in relatorio" :key="index">
                ...
                <td class="text-center td-relatorio">
                  {{ item.dataAgendamento | formatDateTime }}
                </td>
              </tr>
            </tbody>
          </table>
...
import { mesNome, formatSqlDate, orderBy } from '../../../util/util'
import { clientesFechados } from '../../../services/relatorios'
...
export default {
  ...
  methods: {
    ...
    async carregarRelatorio(datas) {
      ...
      const lista = await clientesFechados(this.unidadeAtual.id, `${inicio}T00:00:00`, `${fim}T23:59:59`)
      this.relatorio = orderBy(lista, 'nomeCliente')
