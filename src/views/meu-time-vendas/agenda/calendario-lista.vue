          ...
          <div class="text-center">
            <div class="q-ma-md">
              <app-calendario ref="calendario" :btnUpdate="true" :loading="carregando" :disabled="verificaBloqueio"
                :color="verificaData" @input="evento" @current-date=carregaLista @update-press=carregaLista>
                <div class="q-ml-sm">
        ...
        <div class="col-md-6 col-xs-12 q-pa-md">
          <q-input v-if="!$q.platform.is.mobile" v-model="termo" placeholder="Buscar">
            <template v-slot:prepend>
              <q-select v-model="tipoBusca" :options="['Todos', 'Consultor', 'Paciente', 'Data']" />
            </template>
          ...
          <div v-if="$q.platform.is.mobile">
            <q-select v-model="tipoBusca" label="Tipo da busca" :options="['Todos', 'Consultor', 'Paciente', 'Data']" />
            <q-select v-model="filtro" label="Filtro" :options="filtros">
              <template v-slot:selected>
                {{ filtro ? filtro : '' }}
                <q-badge color="primary" class="q-ml-sm" style="margin-right: -1em">
                  {{ listaFiltrada.length }}
...
import {
  QCard, QInput, QSelect, QIcon, QBadge,

} from 'quasar'
import { retornaPorUnidadeMesAnoTimeVendas } from '../../../services/venda-agenda'
...
export default {
  ...
  methods: {
    ...
    async carregaLista(mes, ano) {
      ...
      const lista = await retornaPorUnidadeMesAnoTimeVendas(this.idUnidadeAtual, mes, ano)
